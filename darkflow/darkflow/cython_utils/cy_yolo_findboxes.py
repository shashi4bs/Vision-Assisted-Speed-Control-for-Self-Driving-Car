...
def yolo_box_constructor(meta,net_out,threshold):
    ...
    probs[:,::1] =  np.ascontiguousarray(net_out[0 : prob_size]).reshape([SS,C])
    confs[:,::1] =  np.ascontiguousarray(net_out[prob_size : (prob_size + conf_size)]).reshape([SS,B])
    coords[: , : ,::1] =  np.ascontiguousarray(net_out[(prob_size + conf_size) : ]).reshape([SS, B, 4])
    final_probs[:,:,::1] = np.zeros([SS,B,C],dtype=np.float32)
    '''
